# Baker Trails — Negotiation Playbook
*5071/5081/5087 Baker Blvd, Baker | Ask $750,000 | Justen Rider, Cornerstone Mgmt Group, (225) 571-3932*
*Prepared 2026-08-02. Numbers from verified pymodel, hedonic model, and calibration DB.*

## Your numbers
- **Walk-away: $499,000** (13% floor) — and this one is special: the market approach independently says **$500K** (real trades: Family Manor Baker $42.1K/unit, Huron St $37.9K/unit 2026). Two methods, same number. You are not lowballing; you are quoting the market.
- Target zone: **$395,000 - $460,000** (16-22%)
- At their $750K ask: 2.7% IRR, P(≥13%) = 0.0%. The ask is ~50% above transaction evidence.
- Already cut once: $855K → $750K. Sellers who cut, cut again.

## Their position
- Rider is both listing agent AND likely the property manager (Cornerstone manages in Baker) — he knows the real collections. Discrepancies between his docs and his marketing are informative.
- Fresh-ish listing at current price but property has been marketed since early 2026 (MLS 2026002777 dated March). Concessions on their own rental ads (1 month free) = soft demand they know about.
- 12 units in Baker = mom-and-pop buyer pool. Your documented-underwrite approach is rare here.

## Scenario scripts

**A. Rider sends docs.**
Underwrite in 48h. The laundromat income is the swing item — if it's real ($500+/mo documented), honest value moves up ~$40-60K. Then call: "I'll be straight — the market for this product in Baker is trading around $40K a door and I can show you the closed sales. I'm at **$460,000** if the T-12 supports what you sent." Name the comps (Family Manor, Huron St). A number with evidence attached is hard to be insulted by.

**B. Rider asks your budget before sending docs.**
"I size to the property, not the other way around. Send the actuals and you'll get a documented number inside two days — I don't waste listing agents' time." (He's also a PM — add: "and however this goes, I'll need management in Baker eventually, so I'd rather be straight with you.")

**C. 'Seller needs $700K+.'**
"Then we're not close, and that's fine — but do me a favor and keep my number. This property has been repriced once already; if it happens again, I can close in 45 days with documents already done." Leave the door open; stale listings come back. Check in at 30-day intervals, never sooner.

**D. Rider hints seller fatigue / 'make an offer.'**
Open at **$425,000** with the comp sheet attached to the LOI, settle target $460-480K. Below $499K everything works; $500-530K only if the T-12 reveals upside (real laundromat income, under-market rents with leases expiring soon).

## Relationship note
Rider matters beyond this deal: Cornerstone manages small multifamily in your core submarket. Every interaction should leave him thinking "serious, fast, straight" — even a dead deal here buys deal flow later. If this dies, ask: "What else do you or your owners have that never hits LoopNet?"

## Hard gates before ANY offer
1. Insurance quote (tornado: insurance is ±13-16pts of IRR on this thin deal — the single biggest input)
2. T-12 + actual rent roll (ours is ESTIMATED — the IC memo flags it)
3. Laundromat income documentation (8 machines — could be $0 or $8K/yr)
4. LLC formed; offer as "Ridgeback Peak Properties LLC or assigns"
